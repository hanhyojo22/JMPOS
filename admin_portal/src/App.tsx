  ...
  return (
    <div className="app-shell">
      <aside className="sidebar">
        ...
        <button className="signout" onClick={() => supabase.auth.signOut()}><LogOut/>Sign out</button>
      </aside>
      <main>
        <header><div><h1>{page === "dashboard" ? "License overview" : "Licenses"}</h1><p>Manage subscriptions, devices, and customer access.</p></div><button className="primary" onClick={() => setCreating(true)}><Plus/>New license</button></header>
...
function CreateModal({ close, created }: { close:()=>void; created:()=>void }) { const [label,setLabel]=useState("");const [slots,setSlots]=useState(1);const [months,setMonths]=useState(12);const [code,setCode]=useState("");const [error,setError]=useState("");async function submit(e:React.FormEvent){e.preventDefault();try{const r=await adminApi<{code:string}>({action:"create",label,slotLimit:slots,durationMonths:months});setCode(r.code)}catch(e){setError(message(e))}} return <div className="overlay center"><form className="modal" onSubmit={submit}><div className="drawer-head"><h2>New license</h2><button type="button" className="icon-button" onClick={close}><X/></button></div>{code?<><div className="generated"><span>Generated license code</span><strong>{code}</strong><small>Save this now. The plaintext code is shown only once.</small></div><button type="button" className="primary wide" onClick={created}>Done</button></>:<>{error&&<div className="error">{error}</div>}<label>Customer or license label<input required value={label} onChange={e=>setLabel(e.target.value)}/></label><label>Subscription term<select value={months} onChange={e=>setMonths(Number(e.target.value))}><option value={1}>1 month</option><option value={3}>3 months</option><option value={6}>6 months</option><option value={12}>1 year</option></select></label><label>Device slots<input type="number" min={1} max={100} value={slots} onChange={e=>setSlots(Number(e.target.value))}/></label><button className="primary wide">Generate license</button></>}</form></div>; }
